...
EOF
      ...
      if [ ! -f "${config.home.homeDirectory}/.local/state/quickshell/scripts/ai/start_ai_service.sh" ]; then
        ...
        if [ -f "${config.home.homeDirectory}/Workspaces/Config/nixos/modules/home-manager/features/quickshell/config/ii/scripts/ai/start_ai_service.sh" ]; then
          $DRY_RUN_CMD cp -f ${config.home.homeDirectory}/Workspaces/Config/nixos/modules/home-manager/features/quickshell/config/ii/scripts/ai/start_ai_service.sh ${config.home.homeDirectory}/.local/state/quickshell/scripts/ai/
      ...
      if [ ! -f "${config.home.homeDirectory}/Pictures/Wallpapers/default.png" ]; then
        $DRY_RUN_CMD cp -f ${config.home.homeDirectory}/Workspaces/Config/nixos/modules/home-manager/features/quickshell/config/ii/assets/images/default_wallpaper.png ${config.home.homeDirectory}/Pictures/Wallpapers/default.png
      fi
    '';
  };
